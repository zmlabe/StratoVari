...
ncep = np.genfromtxt(directorydata + 'ncep_SSWClimo.txt')
lev = np.genfromtxt(directorydata + 'Levels.txt')
diff = ssw_waccm - ncep
time = np.arange(diff.shape[0])

dataall = [ssw_waccm,ncep,diff]
...
plt.title(r'\textbf{SC-WACCM4 minus NCEP}',fontsize=11)
...
plt.savefig(directoryfigure + 'SSWEvents_Difference_WACCM-NCEP.png',dpi=300)
