...
specific humidity response to historical and preindustrial forcings. All 
...
print('\n' '----Plotting Zonal Mean A - %s----' % titletime)
...
def readDataPeriods(typesimu,varnames,simulations,period):
    ...
    varfuture = varfuture*1000.
    varpast = varpast*1000.
...
limit = np.arange(-0.50,0.5001,0.05)
...
for i in range(len(datas)):
    ...
    cs1 = plt.contour(lat,lev,climo,np.arange(-150,51,1),
                      linewidths=0.5,colors='dimgrey') 
    cs2 = plt.contourf(lat,lev,pval,
                       colors='None',hatches=['///////'])     
    
    cs.set_cmap(cmocean.cm.tarn)
...
cbar.set_label(r'\textbf{Q [g/kg]}',fontsize=8,color='dimgray')
...
plt.savefig(directoryfigure + 'Q_zonalmean_PAMIPcomp_SH_Aug19_%s.png' % periodm,
            dpi=300)