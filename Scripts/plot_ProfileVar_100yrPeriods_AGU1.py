...
######################
letters = [r'300',r'100',r'100',r'100']
for v in range(len(varnames)):
    ...
    if FDRmeth == True:
        ...
        ### Mask variables by their adjusted p-values
        pvalue_C[np.where(pvalue_C < 0.05)] = 1.
        pvalue_C[np.where(np.isnan(pvalue_C))] = 0.
    else:
    ...
    if FDRmeth == True:
        plt.savefig(directoryfigure + 'MeanResponse_300ens_%s_%s_FDR.png' % (
                    varnames[v],period),dpi=300)
    else:
       plt.savefig(directoryfigure + 'MeanResponse_300ens_%s_%s_ttest.png' % (
            varnames[v],period),dpi=300) 
