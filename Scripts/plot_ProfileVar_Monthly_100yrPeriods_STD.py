...
using slices of ensemble means for standard deviation
...
print('\n' '----Plotting Monthly Vertical Profiles STD- %s----' % titletime)
...
def readDataPeriods(varnames,sliceq):
    ...
    stdpim = np.nanstd(anompi,axis=0)
    zdiffruns = stdpim
...
for v in range(len(varnames)):
    ...
    if varnames[v] == 'U':
        ...
        limit = np.arange(0,10.1,0.1)
        barlim = np.arange(0,11,5)
    elif varnames[v] == 'GEOP':
        limit = np.arange(0,200.1,1)
        barlim = np.arange(0,201,100)
    ...
    for i in range(len(zdiffruns)):
        ...
        cs = plt.contourf(lat,lev,zdiffruns[i],limit,extend='max')
...
#                     linewidth=5)   
    ...
    if varnames[v] == 'U':
        cbar.set_label(r'\textbf{std.dev. [m/s]}',fontsize=9,color='dimgray',
                       labelpad=0)
    elif varnames[v] == 'TEMP':
        cbar.set_label(r'\textbf{std.dev. [$^\circ$C]}',fontsize=9,color='dimgray',
                       labelpad=0)
    elif varnames[v] == 'GEOP':
        cbar.set_label(r'\textbf{std.dev. [m]}',fontsize=9,color='dimgray',
                       labelpad=0)
    ...
    plt.savefig(directoryfigure + '%s_MonthlyProfiles_100yr_STD.png' % varnames[v],
                dpi=300)
