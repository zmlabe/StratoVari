...
the ensemble mean (300) to calculate the signal-to-noise ratio for AMS
presentation
...
for v in range(len(varnames)):
    ...
    if varnames[v] == 'T2M':
        ...
        limit = np.arange(0,1.1,0.1)
        barlim = np.arange(0,2,1)
    elif varnames[v] == 'U10' or varnames[v] == 'U300' or varnames[v] == 'U700':
        limit = np.arange(0,1.1,0.1)
        barlim = np.arange(0,2,1)
    ...
    for i in range(len(sig)):
        ...
        if i < 6:
            ax1.annotate(r'\textbf{%s}' % labelmonths[i],
                        ...
                        fontsize=20,color='w',rotation=0,
                        ha='center',va='center')
    ...
    plt.savefig(directoryfigure + '%s_SignalToNoise_Maps_AMS.png' % varnames[v],
                dpi=900)
