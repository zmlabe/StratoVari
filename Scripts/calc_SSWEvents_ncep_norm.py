...
def readData(exp):
    ...
    if exp == 'ncep':
        experiment = 'ncep'
        years = np.arange(1948,2015+1,1)
        NENS = len(years)
        
    directorydata = '/seley/zlabe/' + experiment + '/daily/'
    
    ### Read in geopotential polar cap data
    polarcap = np.empty((NENS-1,365*2,17))
    for i in range(NENS-1):
        filepath = directorydata + '%s/' % years[i]
        filename = filepath + 'GEOP_%s_Extended_polarmean.nc' % years[i]
    ...
    for i in range(NENS-1):
        filepath = directorydata + '%s/' % years[i]
        filename = filepath + 'sswcount_%s_11-3_winter.txt' % years[i]
    ...
    for i in range(NENS-1):
        filepath = directorydata + '%s/' % years[i]
        filename = filepath + 'sswstats_%s_11-3_winter.txt' % years[i]
...
def calculateNorm(var):
    ...
    anom = (var - mean)/np.nanstd(var,axis=0)
    
    print('Completed: Calculate normalized anomalies for polar cap!')
    return anom
    
### Call functions for data
polarcap,count,stat,lev = readData('ncep')
anomgeo = calculateNorm(polarcap)
...
plt.title(r'\textbf{Climatology of SSW Events -- Polar Cap Heights -- NCEP/NCAR R1}',fontsize=11)
...
cbar.set_label(r'\textbf{Normalized GEOP}',fontsize=9,color='k',
               labelpad=0)
...
plt.savefig(directoryfigure + 'SSWEvents_ncep_norm.png',dpi=300)
