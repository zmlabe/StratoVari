...
def readHeatFlux(vari):
    ...
    ### Rearrange months (N,D,J,F,M,A)
    rnetdiff = np.nanmean((rnet_sitf - rnet_sith),axis=0)
...
anomsic[np.where(anomsic == 0.)] = np.nan
anomheat = rnet
anomsit = sitf - sith
...
for v in range(len(varq)):
    ...
    if varnamesn[v] == 'SITF':
        ...
        limit = np.arange(-2,0.1,0.2)
        barlim = np.arange(-2,1,1)
    elif varnamesn[v] == '$\Delta$SIC':
        limit = np.arange(-50,1,5)
        barlim = np.arange(-50,1,25)
    elif varnamesn[v] == '$\Delta$NETFLX':
        limit = np.arange(-50,50.1,0.25)
    ...
    if v < 6:
        ax.annotate(r'\textbf{%s}' % labelmonths[v],
                    ...
                    fontsize=15,color='w',rotation=0,
                    ha='center',va='center')
...
#            color='dimgrey',fontsize=6)
    ...
    if v == 5:
        ...
        if varnamesn[v] == '$\Delta$SIC':
            cbar.set_label(r'\textbf{\%}',fontsize=7.5,color='darkgrey')
        elif varnamesn[v] == '$\Delta$NETFLX':
            cbar.set_label(r'\textbf{W/m$^{2}$}',fontsize=7.5,color='darkgrey')  
        else:
            cbar.set_label(r'\textbf{m}',fontsize=7.5,color='darkgrey')
...
plt.savefig(directoryfigure + 'HeatFluxExchanges_Diff_SITSIC_AMS.png',dpi=900)
